...
from prompt import DEFAULT_PROMPT, DISCUSSION_PROMPT, TEACHING_PROMPT, ARGUMENT_PROMPT, INTERVIEW_PROMPT,VALIDATION_PROMPT
...
class PodcastGenerator:
    ...
    def generate_dialogue(self, text: str,prompt_type: str,len:str,minutes:str,count:str) -> List[DialogueItem]:
        ...
        background=copy.deepcopy(prompt)
        background = background.replace("{text}", "")
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                dialogues = self._parse_dialogue(content)

                # 检查生成对话是否合规
                if self._validate_dialogue(dialogues,len,minutes,count,background):
                    print("生成的对话符合要求！")
                    return dialogues
                else:
                    print(f"生成的对话不符合要求，正在重新生成（尝试 {attempt + 1}/{self.max_retries})...")
                    continue
    ...
    def _validate_dialogue(self, dialogues: List[DialogueItem],len:str,minutes:str,count:str,background:str) -> bool:
        dialogue_text = "\n".join([f"[{d.speaker}]: {d.text}" for d in dialogues])
        print(dialogue_text)
        validation_prompt = VALIDATION_PROMPT.replace("{dialogue}", dialogue_text)
        # validation_prompt = VALIDATION_PROMPT.replace("{background}", background)
        # validation_prompt = VALIDATION_PROMPT.replace("{len}", len)
        # validation_prompt = VALIDATION_PROMPT.replace("{minutes}", minutes)
        # validation_prompt = VALIDATION_PROMPT.replace("{count}", count)
        ...
        try:
            ...
            if "合规" in content or "通过" in content:
                return True
            else:
                return False

        except Exception as e:
            raise RuntimeError(f"对话检查失败: {str(e)}")
    ...
    def synthesize_audio(self, dialogues: List[DialogueItem], output_path: str):
        ...
        for idx, dialogue in enumerate(dialogues):
            ...
            try:
                ...
                if idx > 0:
                    combined_audio += AudioSegment.silent(duration=500)  # 添加500ms的对话间隔
...
# python notelm_react.py --input /home/lzj/LLM/tts/doc/朝闻道.pdf --output_dir /home/lzj/LLM/tts/speach
# python notelm_react.py --input /home/lzj/LLM/tts/doc/朝闻道.pdf --output_dir /home/lzj/LLM/tts/speach --prompt_type discussion  teaching  argument  interview
# python notelm_react.py --input /home/lzj/LLM/tts/doc/朝闻道.pdf --output_dir /home/lzj/LLM/tts/speach --prompt_type default  --len 150 --minutes 4 --count 30
# python notelm_react.py --input /home/lzj/LLM/tts/doc/HTTPS.pdf --output_dir /home/lzj/LLM/tts/speach --prompt_type interview  --len 100 --minutes 4 --count 30